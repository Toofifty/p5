...
public class sine_rings extends PApplet {
...
public void draw () { 
  ...
  for (int n = num_rings-1; n >= 0; n--) {
    ...
    rings[n].set_c(0.25f * sin(PI * (t + n * stretch) / 10 / fps) + 0.25f);   // change ring size with t, n, and stretch
  }
  t++;                                                                      // increment clock
...
class Ring {
  ...
  public void set_c (float p) { c = constrain(p, 0, 1); }  // set circumference, constrain from 0 - 1 (just in case)
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sine_rings" };
    if (passedArgs != null) {
