...
// CUSTOM VALUES
public final int BOX_SIZE = 150;  // main box size (sub boxes are /3)
public final int TRI_SIZE = 41;   // triangle size
public final float RATE = 0.0125; // speed of the animation
...
public class Box {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      for (int j = 0; j < n; j++) {
        ...
        if (j % 2 == 0 && i < 3 || j % 2 != 0 && i >= 3) {
          ...
          subTris[c] = new Triangle(TRI_SIZE, x + 1F / 2F / SQ3, j / 2F - y, true);
          
        } else {
          
          subTris[c] = new Triangle(TRI_SIZE, x, j / 2F - y, false);
  ...
  private void drawSubTris() {
    ...
    unIso();
    if (subTris == null) splitTriangles();
